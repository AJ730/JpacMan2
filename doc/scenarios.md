...
```
As a player,
 I want  to play multiple levels;
So that  I can have a longer and more varied game time.

Scenario S5.1: Progress to a new level.
Given  the game has started,
  and  the player has won less than 3 levels;
 When  the player completes a level;
 Then  the game is paused,
  and  a new level is loaded.

Scenario S5.2: Completing all levels.
Given  the game has started,
  and  the player has won 3 levels;
 When  the player completes a level;
